...
TextureRenderer::TextureRenderer(CComPtr<ID3D11Device> pDevice, CComPtr<ID3D11DeviceContext> pContext, int width, int height)
{
...
void TextureRenderer::BeginRender(CComPtr <ID3D11DeviceContext> pDeviceContext, CComPtr<ID3D11Device> &pDevice, float vpWidth, float vpHeight)
{
	...
	if (vpWidth != texWidth || vpHeight != texHeight)
	{
		texWidth = vpWidth;
		texHeight= vpHeight;
		depthStencilBuffer.Release();
		...
		ZeroMemory(&depthStencilDesc, sizeof(depthStencilDesc));
		depthStencilDesc.Width = (UINT)vpWidth;
		depthStencilDesc.Height = (UINT)vpHeight;
		depthStencilDesc.MipLevels = 1;
		depthStencilDesc.ArraySize = 1;
		depthStencilDesc.Format = DXGI_FORMAT_D24_UNORM_S8_UINT;
		depthStencilDesc.SampleDesc.Count = 1;
		...
		ZeroMemory(&textureDesc, sizeof(textureDesc));
		textureDesc.Width = (UINT)vpWidth;
		textureDesc.Height = (UINT)vpHeight;
		textureDesc.MipLevels = 1;
		textureDesc.ArraySize = 1;
		textureDesc.Format = DXGI_FORMAT_R32G32B32A32_FLOAT;
		textureDesc.SampleDesc.Count = 1;
