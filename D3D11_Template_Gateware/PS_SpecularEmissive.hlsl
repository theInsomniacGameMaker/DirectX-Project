...
Texture2D txSpeculer : register(t2);
...
float4 main(PS_INPUT input) : SV_Target
{
	...
	float4 specularTexture = txSpeculer.Sample(samLinear, input.Tex);
	...
	for (int i = 0; i < 6; i++)
	{
		...
		else if (lights[i].Position.w == 1)
		{
			...
			float4 reflectedLight = lights[i].Color * spec* specularTexture.r;

			finalColor += saturate(reflectedLight + directionalLight * specularTexture.x);
		}
		else if (lights[i].Position.w == 2)
		{
			...
			float dotResult = saturate(dot(rVec, viewDir));
			float spec = pow(dotResult, 64);
			float4 reflectedLight = lights[i].Color * spec*specularTexture.x;
